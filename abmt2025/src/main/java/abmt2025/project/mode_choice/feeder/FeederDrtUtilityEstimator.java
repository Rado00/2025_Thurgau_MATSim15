    ...
    public double estimateUtility(Person person, DiscreteModeChoiceTrip trip, List<? extends PlanElement> elements) {
        ...
        try {
            ...
            for (PlanElement element : elements) {
                if (element instanceof Leg) {
                    ...
                    if (mode.equals(TransportMode.drt)) {
                        ...
                        // Estimate waiting time as 10% of travel time (simplified)
            ...
            if (currentCall <= 20 || currentCall % 500 == 0) {
                log.info("FeederDRT utility for person {}: utility={:.3f} (ASC={:.3f}, DRT_time={:.1f}min, " +
                        "DRT_wait={:.1f}min, PT_time={:.1f}min, walk={:.1f}min, cost={:.2f}) [call #{}, success rate: {}/{}]",
                        person.getId(), utility, asc, drtInVehicleTime_min, drtWaitingTime_min,
                        ptInVehicleTime_min, walkTime_min, estimatedCost,
